...
const QuizPageCss = () => {
  ...
  const [userAnswers, setUserAnswers] = useState(Array(cssQuestions.length).fill(''));
  ...
  const calculateScore = () => {
    const correctAnswers = cssQuestions.filter(
      (question, index) => question.correctAnswer === userAnswers[index]
    );
    const score = (correctAnswers.length / cssQuestions.length) * 100;
    return score;
  };
  ...
  const retryQuiz = () => {
    ...
    setUserAnswers(Array(cssQuestions.length).fill(''));
    setTimer(30);
        ...
        {showResults ? (
          ...
          <div>
            ...
            <div className="bg-gray-200 p-4 rounded-md mb-4">
              <p style={{ fontSize: '1.25rem' }}>{cssQuestions[currentQuestionIndex]?.question}</p>
            </div>
            <ul className="list-disc pl-6">
              {cssQuestions[currentQuestionIndex]?.options.map((option, index) => (
                <li
                  key={index}
            ...
            <div className="mt-4">
              ...
              >
                {currentQuestionIndex < cssQuestions.length - 1 ? 'Next Question' : 'Finish Quiz'}
              </button>
            </div>
          </div>
        )}
