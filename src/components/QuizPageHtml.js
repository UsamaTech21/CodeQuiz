// src/components/QuizPageHtml.js
...
const QuizPageHtml = () => {
  ...
  const [userAnswers, setUserAnswers] = useState(Array(htmlQuestions.length).fill(''));
  ...
  const handleNextQuestion = () => {
    clearInterval(timer);
    if (currentQuestionIndex < htmlQuestions.length - 1) {
  ...
  const calculateScore = () => {
    const correctAnswers = htmlQuestions.filter(
      (question, index) => question.correctAnswer === userAnswers[index]
    );
    const score = (correctAnswers.length / htmlQuestions.length) * 100;
    return score;
  };
  ...
  const retryQuiz = () => {
    ...
    setUserAnswers(Array(htmlQuestions.length).fill(''));
    setTimer(30);
        ...
        {showResults ? (
          ...
          <div>
            ...
            <div className="bg-gray-200 p-4 rounded-md mb-4">
              <p style={{ fontSize: '1.25rem' }}>{htmlQuestions[currentQuestionIndex]?.question}</p>
            </div>
            <ul className="list-disc pl-6">
              {htmlQuestions[currentQuestionIndex]?.options.map((option, index) => (
                <li
                  key={index}
            ...
            <div className="mt-4">
              {timer > 0 && <p className="text-gray-600">Time remaining: {timer}s</p>}
              <button
                onClick={timer > 0 ? handleNextQuestion : showResultsPage}
                className="mt-4 bg-blue-500 text-white py-2 px-4 rounded"
              >
                {currentQuestionIndex < htmlQuestions.length - 1 ? 'Next Question' : 'Finish Quiz'}
              </button>
            </div>
          </div>
        )}
